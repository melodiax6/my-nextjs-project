...
const NavCategory: React.FC<NavCategoryProps> = ({ title, children }) => {
    ...
    return (
        <li className="relative list-none group" onClick={toggleDropdown}>
            <span className="cursor-pointer h-10 sm:h-12 px-4 py-2 text-base font-normal rounded-full transition-all duration-300 bg-transparent inline-flex items-center justify-between border-2 text-[#3A3967] border-[#3A3967] dark:text-gray-200 dark:border-gray-200 group-hover:text-[#ff8a65] group-hover:border-[#ff8a65]">
                {title}
                <span
                    className={`ml-2 w-1.5 h-1.5 border-l-2 border-t-2 border-[#3A3967] dark:border-gray-200 transform transition-transform duration-300 ${
                        isOpen ? "rotate-[225deg]" : "rotate-[135deg]"
