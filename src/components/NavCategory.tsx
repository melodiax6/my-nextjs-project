...
import React, { useState, useEffect } from "react";
import { allRecipes } from "@/lib/contentful/api"; // Assuming a function to fetch recipes

// Define types for props and recipe data
type Recipe = {
  fields: {
    id: string;
    title: string;
...
const NavCategory: React.FC<NavCategoryProps> = ({ title, children }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [recipes, setRecipes] = useState<Recipe[]>([]); // Typed recipes state
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        const data: Recipe[] = await allRecipes(); // Ensure `allRecipes` returns a consistent type
        setRecipes(data || []); // Fallback to empty array if `data` is undefined
      } catch (error) {
        console.error("Error fetching recipes:", error);
      }
    };
    ...
    if (title === "Recipes") {
      fetchRecipes(); // Fetch recipes only if the title is "Recipes"
    }
  }, [title]);
        ...
        {title === "Recipes" ? (
          ...
          React.Children.map(children, (child) =>
            React.isValidElement(child) ? (
              <li
                key={(child as React.ReactElement).key || Math.random()} // Use a key if provided or fallback
                className="text-inherit text-base block px-4 py-2 transition-all duration-300"
              >
