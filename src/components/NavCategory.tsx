...
const NavCategory: React.FC<NavCategoryProps> = ({ title, children }) => {
    ...
    return (
        <li className="relative list-none" onClick={toggleDropdown}>
            <span
                className="cursor-pointer h-10 sm:h-12 px-4 py-2 text-base rounded-full transition-all duration-300 bg-transparent inline-flex items-center justify-between border-2 text-[#3A3967] border-[#3A3967] dark:text-gray-200 dark:border-gray-200"
            >
                {title}
                <span
