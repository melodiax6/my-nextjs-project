...
import React, { useEffect, useState } from "react";
import Link from "next/link";
import NavCategory from "./NavCategory"; // Dropdown category navigation
import Search from "./Search"; // Search component
import { ModeToggle } from "./ModeToggle"; // Theme mode toggle component
import { allRecipes } from "@/lib/contentful/api"; // Fetch recipes from Contentful
...
const Navbar: React.FC = () => {
    const [recipes, setRecipes] = useState<Recipe[]>([]); // Recipes state

    useEffect(() => {
        const fetchRecipes = async () => {
            try {
                const data = await allRecipes("recipes");
                setRecipes(data || []); // Handle null by defaulting to an empty array
            } catch (error) {
                console.error("Error fetching recipes:", error);
            }
        };
